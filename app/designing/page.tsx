import type { Metadata } from 'next';
import { Container } from '@/components/ui/Container';
import { ProjectGrid } from '@/components/projects/ProjectGrid';
import { getProjectsByCategory } from '@/lib/supabase/queries';

export const metadata: Metadata = {
    title: 'Designing Human Interfaces',
    description: 'Explore my interface design projects focused on creating intuitive and beautiful digital experiences.',
};

export default async function DesigningPage() {
    const projects = await getProjectsByCategory('designing');

    return (
        <Container className="pt-40 md:pt-60 pb-20">
            <div className="mb-24 max-w-3xl">
                <span className="block font-sans text-xs tracking-[0.3em] uppercase text-indigo-200/60 mb-6">
                    Interface Design
                </span>
                <h1 className="font-serif text-5xl md:text-7xl text-platinum mb-6 italic">
                    Designing Human Interfaces
                </h1>
                <p className="font-sans text-lg text-white/50 leading-relaxed">
                    Creating digital experiences that balance functionality with aesthetic restraint.
                    Each interface is crafted with intention, focusing on clarity, elegance, and human connection.
                </p>
            </div>

            {projects.length > 0 ? (
                <ProjectGrid projects={projects} variant="interface" />
            ) : (
                <div className="text-center py-20">
                    <p className="text-white/40 text-lg">No projects yet. Add some from the admin panel!</p>
                </div>
            )}
        </Container>
    );
}
